...
class Summarizer:
  ...
  def generate_department_report(self, department_name: str, date_from: str, date_to: str):
      try:
        llm_client = LLMClient(agent_type="DEPARTMENT_REPORT_GENERATOR")
      except Exception as e:
        logger.error(f"Failed to initialize LLM client: {e}")
        return {"summary": []}
        
      stats_json = get_department_comprehensive_stats(department_name, date_from, date_to)
      data_str = json.dumps(stats_json, indent=2, ensure_ascii=False)
    
      prompt = self.prompt_builder.build_department_report_prompt(department_name, date_from, date_to, data_str)
    
      return llm_client.complete(prompt).strip()
  
  def generate_manager_report(self, date_from: str, date_to: str):
      try:
        llm_client = LLMClient(agent_type="MANAGER_REPORT_GENERATOR")
      except Exception as e:
        logger.error(f"Failed to initialize LLM client: {e}")
        return {"summary": []}
        
      stats_json = get_manager_general_stats(date_from, date_to)
      data_str = json.dumps(stats_json, indent=2, ensure_ascii=False)
    
      prompt = self.prompt_builder.build_manager_report_prompt(
          date_from=date_from,
          date_to=date_to,
          json_data=data_str
      )

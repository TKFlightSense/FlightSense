...
class DataFilter:
        ...
        if self.label_type:
            if isinstance(self.label_type, str):
                try:
                    ...
                    errors['label_type'] = f"Invalid label_type. Must be one of: {valid_types}"
        ...
        if self.label_status:
            if isinstance(self.label_status, str):
                try:
                    ...
                    errors['label_status'] = "Invalid label_status. Must be 'POSITIVE' or 'NEGATIVE'"
    ...
    def to_enum(self) -> 'DataFilter':
        ...
        if isinstance(self.label_type, str):
            try:
                self.label_type = SentimentLabel[self.label_type.upper().replace('-', '_')]
            except KeyError:

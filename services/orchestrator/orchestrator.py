    ...
    def get_processed_data_filtered(
        ...
        try:
            ...
            if data_filter.label_type:
                if not self.can_access_category(
            ...
            if not data_filter.label_type and not self.is_full_access_role(
                user_info["role"]
            ):
...
if __name__ == "__main__":
    ...
    orchestrator = FlightSenseOrchestrator(
        db, secret_key="your-secret-key-here"
    ...
    # Manager user
    orchestrator.register_user(
        "manager", "manager@flightsense.com", "manager123", "manager"
    )

    # Subject-specific users
    orchestrator.register_user(
        "flight_user", "flight@flightsense.com", "flight123", "flight_delay"
    )
    orchestrator.register_user(
        "baggage_user", "baggage@flightsense.com", "baggage123", "baggage"
    )
    ...
    if login_result["success"]:
        ...
        data = orchestrator.get_processed_data_filtered(
            token, {"label_type": "flight_delay_cancellation"}
        )
        print(f"Access own category: {data['success']}")

        # Try to access baggage category (should fail)
        data = orchestrator.get_processed_data_filtered(
            token, {"label_type": "baggage_issues"}
        )
    ...
    if login_result["success"]:
        ...
        data = orchestrator.get_processed_data_filtered(
            token, {"label_type": "baggage_issues_negative"}
        )
